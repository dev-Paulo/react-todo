    ...
    <div className="h-screen flex flex-col">
      ...
      <section className="bg-gray-600 flex-grow flex-col flex">
        <div className="flex-col mt-[-20px] items-center flex">
          <form onSubmit={handleAddTask} className="flex-row justify-center items-center flex w-2/4">
            <input
              type="text"
              name="taskTitle"
              value={newTaskTitle}
          ...
          <div className="flex-col items-center flex w-2/4">
            {tasks.map((task: Task, index: number) => (
              <div key={task.id} className="h-[72px] mb-4 p-4 rounded-lg bg-gray-500 flex-row inline-flex items-center w-full justify-between">
                ...
                <div className="">
                  <Trash size={24} className="text-gray-300" onClick={() => handleTaskDeletion(index)} />
                </div>
              </div>
            ))}
          </div>
