...
function App() {
  ...
  useEffect(() => {
    setCompletedTasksCount(isTaskCompleted.filter((completed) => completed).length);
  }, [isTaskCompleted]);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="bg-gray-600 flex-grow flex-col flex px-4">
        <div className="flex-col mt-[-20px] items-center flex ">
          <form onSubmit={handleAddTask} className="flex-row justify-center items-center flex w-full md:w-2/4 ">
            <input
              type="text"
              name="taskTitle"
              value={newTaskTitle}
            ...
            <button
              type="submit"
              className="text-center inline-flex items-center text-white bg-blue-600 hover:bg-blue-600 focus:ring-2 focus:ring-blue-300 font-bold rounded-lg text-sm px-5 py-2.5 ml-2 focus:outline-none dark:focus:ring-blue-800"
            >
              Criar
              <PlusCircle size={20} className="ml-2" />
            </button>
          </form>
          <div className="flex-row inline-flex justify-between mt-10 w-full md:w-2/4 mb-4">
          ...
          {tasks.length > 0 ? (
            ...
            {tasks.map((task: Task, index: number) => (
              <div key={task.id} className="h-[72px] mb-4 p-4 rounded-lg bg-gray-500 flex-row inline-flex items-center w-full justify-between">
                <div className="flex-row inline-flex items-center">
                  ...
                  <p
                    ...
                    style={{
                      textDecoration: `${isTaskCompleted[index] ? "line-through" : ""}`,
                      color: `${isTaskCompleted[index] ? "#808080" : "#F2F2F2"}`,
                    }}
                  >
                    {task.name}
                  </p>
                ...
                <div className="">
                  <Trash size={24} className="text-gray-300 cursor-pointer" onClick={() => handleTaskDeletion(index)} />
                </div>
              </div>
            ))}
          </div>
          ...
          (
            <div className="flex-col items-center flex w-full md:w-2/4 mt-4 text-center">
              ...
              <p className="font-normal text-gray-300">Crie tarefas e organize seus itens a fazer</p>
            </div>
          )}
